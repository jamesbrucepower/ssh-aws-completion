(function() {
	...
	var InstanceCompletion = function() {
		...
		this.CACHED_LB_LIST = '/tmp/cache.lb';
	...
	_.extend(InstanceCompletion.prototype, {
		...
		createConfig: function() {
			...
			if (!accessKey || !secretKey) {
				console.error("You must define AWS_ACCESS_KEY_ID & AWS_SECRET_ACCESS_KEY environment variables");
				process.exit(1);
			} else
				return {
		...
		cacheLoadBalancerList: function(list) {
			fs.writeFileSync(this.CACHED_LB_LIST, JSON.stringify(list));
		},
		...
		getCachedLoadBalancerList: function() {
			if (fs.existsSync(this.CACHED_LB_LIST)) {
				var stats = fs.statSync(this.CACHED_LB_LIST);
				if (stats.isFile() && this.isRecentlyCached(stats.mtime)) {
					console.error('reading from cache');
					var data = fs.readFileSync(this.CACHED_LB_LIST, 'utf8');
		...
		getLoadBalancers: function(loadBalancerName, cb) {
			...
			this.elb.describeLoadBalancers(params, function(err, data) {
				if (err) {
					...
					self.cacheLoadBalancerList(list);
					cb(null, list);
				}
			});
		},
		...
		/*getSubnets: function(subnets, cb) {
			var params = {
				"SubnetIds": subnets
			};
			ec2.describeSubnets(params, function(err, data) {
				if (err) {
					...
					cb(null, _.pluck(data.Subnets, 'SubnetId'));
				}
			});
		},*/
		...
		getNatInstanceIdsForSubnets: function(subnets, cb) {
			...
			self.getRouteTable(params, function(err, defaultRouteTable) {
				if (err) {
					...
					_.each(defaultRouteTable, function(v, k) {
						subnets[k] = subnets[k] || {};
						_.extend(subnets[k], v);
					});
					...
					self.getRouteTable(params, function(err, routeTable) {
						if (err) {
							...
							_.each(routeTable, function(v, k) {
								subnets[k] = subnets[k] || {};
								_.extend(subnets[k], v);
							});
		...
		getSubnets: function(instances) {
			...
			_.each(instances, function(i) { 
				if (typeof(i.SubnetId) !== 'undefined')
					result[i.SubnetId] = {
						"vpcId": i.VpcId
					};
			});
