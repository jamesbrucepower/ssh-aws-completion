...
var AWSCompletion = function(options) {
	...
	if (options) {
		this.input = options.input || argv._.shift();
		this.offset = options.offset || this.input.length;
	}
}
		...
		if (!self.showError(err)) {
			...
			_.each(self.instances, function(instance, id) {
				...
				if (options.metaChar == self.metaChars.instance) 
					match = [id, self.loadBalancerMap[id]];
				else 
		...
		if (!self.showError(err)) {
			...
			if (options && options.filter && options.filter.length > 0) {
				...
				if (options.metaChar == self.metaChars.lb) 	
					matches = self.filterByPrefix(options.filter, _.keys(loadBalancers));
				else 
					matches = self.filterByPrefix(options.filter, _.keys(self.loadBalancerMap));
			...
			// If there is no filter show all load balancers
				...
				if (options.metaChar == self.metaChars.instance) 
					self.output(options, _.keys(self.loadBalancerMap));
				else 
